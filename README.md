...
2. Some calls are kept by rule, without a question: failed calls, calls of
   tools that cannot be re-run (`protectTools`), and edits (`editTools`), which
   are small and record what changed.
...
| `editTools` | Edit, Write, MultiEdit, NotebookEdit, edit, write, apply_patch | Tools that change files; their calls are always kept; replaces the list |
...
Numbers from [benchmarks/RESULTS.md](benchmarks/RESULTS.md), produced by
`npm run bench` against live Jev on the sample sessions in
[benchmarks/samples/](benchmarks/samples/) (synthetic, shaped like real agent
work, regenerated by `benchmarks/make-samples.ts`) and, as an aggregate only,
on six real Claude Code sessions from one developer's machine, cut at the
first user turn past ~120k tokens.

Three configurations: **default** (this project), **recoverable** (the
original fast-jev-compaction behavior: its question wording, one 0.5
threshold, no result excerpts, no protection rules), and **drop-all** (every
answer 0, the rules still applied: what an engine without judgment would do).

| Session | Config | Scored | Kept | Truncated | Dropped | Protected | Fewer chars | Needed-later lost |
| --- | --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| bugfix-checkout (38 msgs) | default | 10 | 3 | 0 | 7 | 3 | 56% | 1/1 |
| | recoverable | 12 | 0 | 0 | 12 | 1 | 85% | 1/1 |
| | drop-all | 10 | 0 | 0 | 10 | 3 | 80% | 1/1 |
| incident-api-latency (38 msgs) | default | 11 | 1 | 2 | 8 | 2 | 18% | 0/0 |
| | recoverable | 11 | 0 | 1 | 10 | 2 | 22% | 0/0 |
| | drop-all | 11 | 0 | 0 | 11 | 2 | 27% | 0/0 |
| research-rate-limiting (32 msgs) | default | 9 | 2 | 0 | 7 | 2 | 90% | 1/1 |
| | recoverable | 11 | 0 | 0 | 11 | 0 | 98% | 1/1 |
| | drop-all | 9 | 0 | 0 | 9 | 2 | 94% | 1/1 |
| long-refactor-db-layer (179 msgs) | default | 61 | 1 | 1 | 59 | 22 | 82% | 0/0 |
| | recoverable | 82 | 0 | 0 | 82 | 1 | 94% | 0/0 |
| | drop-all | 61 | 0 | 0 | 61 | 22 | 85% | 0/0 |
| **6 real sessions, 1311 msgs, cut at ~120k tokens** | default | 440 | 197 | 21 | 222 | 133 | 5–29% | **18/62** |
| | recoverable (rules on) | 440 | 0 | 1 | 439 | 133 | 37–93% | 62/62 |
| | drop-all | 440 | 0 | 0 | 440 | 133 | 37–93% | 62/62 |

Every sample compacted in one Jev request, 0.4 to 1.5 s. The real sessions
took one request each (one took two) and reached the `inputs<=200` and
`texts abridged` fitting stages; details in
[benchmarks/REAL-SESSIONS.md](benchmarks/REAL-SESSIONS.md).

Read it like this. The original wording with a 0.5 threshold drops everything
it scores, on every sample and every real session; the engine flags that as
"no keep signal" and does not apply it. The defaults keep what Jev judges
still useful, so the reduction is lower and the result is a history the model
can work from. On the real sessions the defaults lose 18 of the 62 results the
needed-later proxy flags, against all 62 for the original behavior, at a
lower reduction. The decision listings in the results file show every call
with its scores, misses included.
