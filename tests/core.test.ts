...
import { applyDecisions, batchCalls, buildJevRequest, collectToolCalls, compact, compactMessages, decideCall, estimateTokens, fitState, JevClient, parseJevResponse, questionsFor, reductionRatio, resolveOptions, type HistoryToolCall, type JevAsker, type JevQuestions, type Message, type ToolCall } from '../src/core/index.js';
...
function transcript(): Message[] {
  return [
    ...
    result('tool-3', 'FAIL b.test.ts: expected 2 to be 3'),
...
describe('options', () => {
  it('fills in defaults and ignores non-finite values', () => {
    ...
    expect(resolveOptions({ keepThreshold: 0.4 })).toMatchObject({ keepCallThreshold: 0.4, keepResultThreshold: 0.4 });
    expect(resolveOptions({ keepThreshold: 0.4, keepResultThreshold: 0.1 })).toMatchObject({ keepCallThreshold: 0.4, keepResultThreshold: 0.1 });
...
describe('tool call collection', () => {
  it('pairs each tool call with its result and pins recent ones', () => {
    ...
    expect(calls[2]?.isError).toBe(false);
...
describe('state fitting', () => {
  it('sends the whole history with tool results replaced by a note', () => {
    ...
    expect((state.history[4]?.tool_calls?.[0] as HistoryToolCall).result).toMatch(/^ok, /);
  });
  ...
  it('abridges long texts oldest-first and collapses old messages last', () => {
    ...
    const collapsed = fitState(messages, [], { ...fit, maxStateTokens: 460, preserveRecentMessages: 1 });
    expect(collapsed.stage).toBe('old messages collapsed');
    expect(collapsed.tokens).toBeLessThanOrEqual(460);
...
describe('question batching', () => {
  ...
  it('splits questions across requests when the state leaves little room', () => {
    const batches = batchCalls(calls, 29_400, { ...options, questionStyle: 'recoverable' });
...
describe('decisions', () => {
  ...
  it('removes dropped calls and truncates dropped results', () => {
    ...
    expect(kept[2]?.toolUses[0]?.text).toMatch(
      new RegExp(`^${'x'.repeat(300)}\\n\\[jev-compaction truncated 1700 chars`),
    );
  ...
  it('honours truncateHeadChars, including a zero head', () => {
    ...
    expect(kept[2]?.toolResults?.[0]?.text).toBe(
      `${original.slice(0, 50)}\n[jev-compaction truncated ${total - 50} chars of this tool result; re-run the tool if needed]`,
    );
    ...
    expect(noHead[2]?.toolResults?.[0]?.text).toBe(
      `[jev-compaction truncated ${total} chars of this tool result; re-run the tool if needed]`,
    );
  });
});
...
describe('compact', () => {
  it('resends the full state with every batch and merges the answers', async () => {
    ...
    const questionPairTokens = estimateTokens(JSON.stringify(questionsFor(collectToolCalls(messages, 1)[0]!)));
    const output = await compact(
      messages,
      fakeJev((name) => (name.startsWith('call_') ? 0.9 : 0.1), seen),
      { preserveRecentMessages: 1, maxRequestTokens: stateTokens + 20 + questionPairTokens * 2 + 10 },
    );
